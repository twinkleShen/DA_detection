...
def parse_args():
    ...
    parser.add_argument('--dataset', dest='dataset',
                        ...
                        default='cityscape', type=str)  # airplane
    ...
    parser.add_argument('--save_dir', dest='save_dir',
                        help='directory to save models', default="./data/output_model",
                        type=str)
...
if __name__ == '__main__':
    ...
    if args.dataset == "cityscape":
        ...
        print('loading our dataset...........')
        args.s_imdb_name = "airplane_2007_train_s"
        args.t_imdb_name = "airplane_2007_train_t"
        args.s_imdbtest_name = "airplane_2007_test_s"
        args.t_imdbtest_name = "airplane_2007_test_t"
        args.set_cfgs = ['ANCHOR_SCALES', '[4,8,16,32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '50']
    ...
    if args.resume:
        load_name = os.path.join(output_dir,
                                 'FLIR_faster_rcnn_{}_{}_{}.pth'.format(args.checksession, args.checkepoch, args.checkpoint))
    ...
    if args.use_tfboard:
        ...
        logger = SummaryWriter("tensorboard_DA")
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
                   ...
                   + args.lamda * (DA_img_loss_cls.mean() + DA_ins_loss_cls.mean() \
                                   + tgt_DA_img_loss_cls.mean() + tgt_DA_ins_loss_cls.mean() + 10*DA_cst_loss.mean() + tgt_DA_cst_loss.mean())
            loss_temp += loss.item()
            ...
            if step % args.disp_interval == 0:
                ...
                if args.mGPUs:
                    ...
                    DA_img_loss_cls = DA_img_loss_cls.item()
                    tgt_DA_img_loss_cls = tgt_DA_img_loss_cls.item()
                    loss_DA_img_cls = args.lamda * (DA_img_loss_cls + tgt_DA_img_loss_cls) / 2
                    DA_ins_loss_cls = DA_ins_loss_cls.item()
                    tgt_DA_ins_loss_cls = tgt_DA_ins_loss_cls.item()
                    loss_DA_ins_cls = args.lamda * (DA_ins_loss_cls + tgt_DA_ins_loss_cls) / 2
                    DA_cst_loss = DA_cst_loss.item()
                    tgt_DA_cst_loss = tgt_DA_cst_loss.item()
                ...
                if args.use_tfboard:
                    ...
                    logger.add_scalars("loss_s_{}".format(args.session), info,
                                       (epoch - 1) * iters_per_epoch + step)
                    info2 = {
                        'DA_img_loss_cls': DA_img_loss_cls,
                        'tgt_DA_img_loss_cls': tgt_DA_img_loss_cls,
                        'loss_DA_img_cls': loss_DA_img_cls,
                    }
                    logger.add_scalars("img_loss_{}".format(args.session), info2,
                                       (epoch - 1) * iters_per_epoch + step)

                    info3 = {
                        'DA_ins_loss_cls': DA_ins_loss_cls,
                        'tgt_DA_ins_loss_cls': tgt_DA_ins_loss_cls,
                        'loss_DA_ins_cls': loss_DA_ins_cls,
                    }
                    logger.add_scalars("ins_loss_{}".format(args.session), info3,
                                       (epoch - 1) * iters_per_epoch + step)

                    info4 = {
                        'DA_cst_loss': DA_cst_loss,
                        'tgt_DA_cst_loss': tgt_DA_cst_loss,
                        'loss_DA_cst': loss_DA_cst,
                    }
                    logger.add_scalars("cst_loss_{}".format(args.session), info4,
                                       (epoch - 1) * iters_per_epoch + step)
                loss_temp = 0
                start = time.time()

        # if epoch == args.max_epochs:
        # save_name = os.path.join(output_dir, 'airplane_consist_default.pth'.format(args.session, epoch, step))
        # save_checkpoint({
        ...
        # few_FLIR :checksession = 1
        # FLIR :checksession = 2
        save_name = os.path.join(output_dir, 'sess_10_FLIR_faster_rcnn_{}_{}_{}.pth'.format(args.session, epoch, step))
        save_checkpoint({
            'session': args.session,
            'epoch': epoch + 1,
            'model': fasterRCNN.module.state_dict() if args.mGPUs else fasterRCNN.state_dict(),
